...
sys_vendas
WHERE
MES1 > 0  and idvendedor = " . $_SESSION['idrepresentante'];
if ($cliente <> 1) {
...
sys_vendas
WHERE
MES2 >0 and idvendedor = " . $_SESSION['idrepresentante'];
if ($cliente <> 1) {
    $query .= " and idcliente IN($cliente) ";
}
$query .= " GROUP BY idindustria
...
sys_vendas
WHERE
MES3 >0  and idvendedor = " . $_SESSION['idrepresentante'];
if ($cliente <> 1) {
    $query .= " and idcliente IN($cliente) ";
}
$query .= " GROUP BY idindustria
...
sys_vendas
WHERE
MES4 >0  and idvendedor = " . $_SESSION['idrepresentante'];
if ($cliente <> 1) {
    $query .= " and idcliente IN($cliente) ";
}
$query .= " GROUP BY idindustria
...
sys_vendas
WHERE
MES5 >0  and idvendedor = " . $_SESSION['idrepresentante'];
if ($cliente <> 1) {
    $query .= " and idcliente IN($cliente) ";
}
$query .= " GROUP BY idindustria
...
if ($industriaselecionada = 1) {
    $query .= " and a.idindustria IN(select distinct idindustria from sys_vendas where idvendedor = ".$_SESSION['idrepresentante'].")";
}else{
...
while ($row = pg_fetch_array($result)) {
   // if ($row['periodo'] > 0) {
        ...
        if ($row['mix'] == 0) {
            ...
            echo '<td align="center" > ' . $row['mix'] . '</td>';
        }
        if ($row['mes5'] == 0) {
