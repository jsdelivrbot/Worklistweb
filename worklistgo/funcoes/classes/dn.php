...
while ($exibe2 = pg_fetch_array($sql2)) {
    ...
    } else if (strcasecmp($exibe2['grupo'], 'PRO-01') == 0) {
...
//usar para dias corridos
$date = date('d/m/Y');
$mes = date('m');
$mes = buscarDiasMes($mes);

//primeiro dia do mes
$dataAux = $mes . date('Y');
//ultimo dia do mes
$i = date('m');
$ano = buscarUltimoDia($i);
$dataAux2 = $ano . date('Y');

$diasCorridos = DiasUteis($dataAux, $date);
$diasCorridos = $diasCorridos+1;

$diasUteis = DiasUteis($dataAux, $dataAux2);
$diasUteis = $diasUteis-1;


$sql_geral_dn = "select ROUND(SUM(a.total),2) as total,COUNT(DISTINCT a.cnpj) AS dn"
        . " from sys_farol_danone a where a.mes_venda >='$inicio_mes' and a.mes_venda<='$final_mes' and a.id_rca=" . $_SESSION['idrepresentante'] . " and a.tipo_vendedor='VENDEDOR'";
...
$sql3 = "select a.divisao, SUM(a.total) AS TOTAL, SUM(a.dn) AS DN from (select a.divisao, SUM(a.total) as total, COUNT(DISTINCT a.cnpj) AS dn
        from sys_farol_danone a where id_rca=" . $_SESSION['idrepresentante'] . " and a.mes_venda >= '$inicio_mes' and a.mes_venda <='$final_mes' and tipo_vendedor='VENDEDOR' 
        group by a.divisao union all select distinct a.divisao, 0 as total, 0 as dn from sys_farol_danone a)a group by a.divisao ORDER BY a.DIVISAO asc";
...
//soma realizado
//$total_realizado = $imf_realizado + $cereal_realizado + $sustain_realizado + $gum_realizado+$profutura_realizado;
...
//soma dn
// $dn_realizado = $imf_realizado_dn + $cereal_realizado_dn + $sustain_realizado_dn + $gum_realizado_dn+$profutura_realizado_dn;
//$perc_total_dn = $perc_imf_dn + $perc_cereal_dn + $perc_sustain_dn + $perc_gum_dn+$perc_profutura_dn;
if(isset($geral_dn)){
