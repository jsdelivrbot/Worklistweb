...
to_char(CAST(CURRENT_DATE AS DATE) + INTERVAL '-5 MONTH','yyyy-mm-01') AS dtini,
...
D.IDCLI, 
D.CLIENTE, 
SUM(D.TOTAL) AS APT1, 0 AS APT2, 0 AS APT3, 0 AS APTAR, 0 AS APTSJ, 0 AS PRO1, 0 AS GUM
FROM 
sys_farol_danone D
where
...
D.IDCLI, 
D.CLIENTE, 
0 AS APT1, SUM(D.TOTAL) AS APT2, 0 AS APT3, 0 AS APTAR, 0 AS APTSJ, 0 AS PRO1, 0 AS GUM
FROM 
sys_farol_danone D
where
...
D.IDCLI, 
D.CLIENTE, 
0 AS APT1, 0 AS APT2, SUM(D.TOTAL) AS APT3, 0 AS APTAR, 0 AS APTSJ, 0 AS PRO1, 0 AS GUM
FROM 
sys_farol_danone D
where
...
D.IDCLI, 
D.CLIENTE, 
0 AS APT1, 0 AS APT2, 0 AS APT3, SUM(D.TOTAL) AS APTAR, 0 AS APTSJ, 0 AS PRO1, 0 AS GUM
FROM 
sys_farol_danone D
where
...
D.IDCLI, 
D.CLIENTE, 
0 AS APT1, 0 AS APT2, 0 AS APT3, 0 AS APTAR, SUM(D.TOTAL) AS APTSJ, 0 AS PRO1, 0 AS GUM
FROM 
sys_farol_danone D
where
D.GRUPO_COBERTURA = 'APT-SJ' AND 
D.MES_VENDA >= '$inicio_mes' AND D.MES_VENDA <= '$final_mes' 
GROUP BY D.IDCLI, D.CLIENTE
UNION ALL
...
D.IDCLI, 
D.CLIENTE, 
0 AS APT1, 0 AS APT2, 0 AS APT3, 0 AS APTAR, 0 AS APTSJ, SUM(D.TOTAL) AS PRO1, 0 AS GUM 
FROM 
sys_farol_danone D
where
...
D.IDCLI, 
D.CLIENTE, 
0 AS APT1, 0 AS APT2, 0 AS APT3, 0 AS APTAR, 0 AS APTSJ, 0 AS PRO1, SUM(D.TOTAL) AS GUM
FROM 
sys_farol_danone D
where
...
if (pg_affected_rows($sql_cm_geral) == 0) {
    ...
    while ($valores = pg_fetch_array($sql_cm_geral)) {
        ...
        if (round($valores['aptsj'], 2) <= 0) {
            echo "<td style='vertical-align:middle; text-align:center;'><i class='fa fa-fw fa-thumbs-o-down' style='color:#FF0000;' title='R$ " . round($valores['aptsj'], 2) . "'></i></td>";
        } else {
            echo "<td style='vertical-align:middle; text-align:center;'><i class='fa fa-fw fa-thumbs-o-up' style='color:#000066;' title='R$ " . round($valores['aptsj'], 2) . "'></i></td>";
            $cm++;
        }
